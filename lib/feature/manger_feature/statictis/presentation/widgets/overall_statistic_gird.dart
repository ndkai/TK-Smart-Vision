...
class OverallStatisticGrid extends StatelessWidget {
  ...
  final String id;
  const OverallStatisticGrid({Key key, this.responseSwaggger, this.id}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    print("OverallStatisticGrid student success");
    if(id == "ToDayOverall"){
      ...
      for(var data in overviews){
        ...
        if(tempday.day == now.day && tempday.month == now.month && tempday.year == now.year ){
          print("OverallStatisticGrid ToDayOverall success");
          studentData = data;
          print("OverallStatisticGrid ToDayOverall success ${studentData.morning.absentStatusNumbers + studentData.afternoon.absentStatusNumbers}");
        }
      }
      return Container(
        ...
        child: Column(
          children: <Widget>[
            Flexible(
              child: id == "ToDayOverall" ? Row(
                children: <Widget>[
                  _buildStatCard('Học sinh', responseSwaggger.data.items[0].statisticOverview.employeeNumbers.toString(), Colors.green),
                  _buildStatCard('Phiếu ăn', responseSwaggger.data.items[0].mealTicketsToDayQuantity.toString(), Colors.green),
    ...
    }  else  if(id == "WeekOverall") {
      print("OverallStatisticGrid WeekOverall load");
