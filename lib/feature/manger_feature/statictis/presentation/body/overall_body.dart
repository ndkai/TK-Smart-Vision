...
class OverallBody extends StatefulWidget {
  final String id;

  const OverallBody({Key key, this.id}) : super(key: key);
  @override
  _OverallBodyState createState() => _OverallBodyState();
}

class _OverallBodyState extends State<OverallBody> {
  @override
        ...
        BlocBuilder<StatisticBloc, StatisticState>(
            builder: (context, state){
              ...
              if(state is Loaded){
                ...
                if(widget.id == "ToDayOverall"){
                  return Column(
                    children: [
                      OverallStatisticGrid(responseSwaggger: state.statisticResponseSwaggger, id: widget.id,),
                      TodayChart(data: state.statisticResponseSwaggger, id: "ToDayOverall",),
                    ],
                  );
                } else
                  
                if(widget.id == "WeekOverall"){
                  return Column(
                    children: [
                      OverallStatisticGrid(responseSwaggger: state.statisticResponseSwaggger, id: widget.id,),
